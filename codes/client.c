...
void receiving_segment(){
    while(1){
        ...
        if(recv_status<0){
            ...
            disconnect_from_server();
            sleep(1);
            execlp("./_peer_server","./_peer_server",temp);
            printf("Error executing _peer_server");
            exit(EXIT_FAILURE);
        }
            ...
            disconnect_from_server();
            sleep(1);
            execlp("./_peer_client","./_peer_client",IP_Addrs[option-1],transfer_port);
            printf("Error executing _peer_client");
            exit(EXIT_FAILURE);
        }
